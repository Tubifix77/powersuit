...
esp_err_t imu_mpu6050_init(imu_mpu6050_t *imu, i2c_master_bus_handle_t bus, uint8_t addr)
{
    ...
    if (err == ESP_OK) {
        err = wr_reg(imu, REG_GYRO_CONFIG, 0x10);     /* +-1000 dps */
    }
    if (err == ESP_OK) {
        err = wr_reg(imu, REG_ACCEL_CONFIG, 0x10);    /* +-8 g */
    }
    if (err != ESP_OK) {
        return err;
    }
    imu->gyr_dps_per_lsb = 1.0f / 32.8f;              /* +-1000 dps scale */
    imu->acc_g_per_lsb = 1.0f / 4096.0f;              /* +-8 g scale */

    if (rd_regs(imu, REG_WHO_AM_I, &who, 1) == ESP_OK) {
        ESP_LOGI(TAG, "imu who_am_i=0x%02x", (unsigned)who);
    }
    return ESP_OK;
}
...
esp_err_t imu_mpu6050_read(imu_mpu6050_t *imu, float acc_g[3], float gyr_dps[3])
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        gyr_dps[i] = (float)g * imu->gyr_dps_per_lsb;
    }
    return ESP_OK;
}
